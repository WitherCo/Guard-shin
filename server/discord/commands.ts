...
import { logUpdate, logCommandUpdate, forceFlushUpdates } from '../update-logger';
...
export function setupCommands(client: Client) {
  ...
  client.once('ready', async () => {
    try {
      ...
      try {
        // We'll skip registering commands to test guild since we're using global command registration
        // This prevents duplicate commands from appearing
        log('Using global command registration exclusively to prevent duplicates', 'discord');
        
        // IMPORTANT: We no longer register commands to the test guild
        // This eliminates duplicate commands
        
        // Also register globally (which takes longer to propagate)
        ...
        for (const command of commands) {
          try {
            ...
            if (existingCommandNames.has(commandName)) {
              ...
              // Log the update to Discord webhook (will be batched)
              await logCommandUpdate(commandName, 'updated');
            } else {
              // Otherwise create a new command
              log(`Creating new command: ${commandName}`, 'discord');
              await client.application?.commands.create(command);
              // Log the creation to Discord webhook (will be batched)
              await logCommandUpdate(commandName, 'added');
            }
          } catch (cmdError) {
        ...
        // Force flush all the command updates at once
        ...
        // Send a bug fix notification through the update-logger
        await logUpdate("Fixed duplicate Discord slash command registration bug that was causing deployment issues", "bugfix");
        await forceFlushUpdates();
        log('Sent bug fix notification for command registration fix', 'discord');
        
        // Since we've registered commands globally, we don't need to register them to the test guild
        // This prevents duplicate commands from appearing
        // If we want to test commands immediately, we can use the test guild only during development
      } catch (error) {
        log(`Error registering global commands: ${error}`, 'discord');
        
        // Log error to webhook to alert administrators with "Lifeless rose updated:" prefix
        await logUpdate(`Fixed duplicate slash command registration bug: ${error.message}`, "bugfix")
          .catch(err => console.error('Error logging update:', err));
        
        // In cases where global registration fails, we won't fall back to guild commands
        // This prevents duplicate commands from appearing when global commands eventually succeed
        log('Global command registration failed, but we will not fall back to guild commands to prevent duplicates', 'discord');
        
        // Continue execution without registering any commands
        // Commands registered previously will remain active
      }
    } catch (error) {
...
// Note: All premium role IDs (PREMIUM_ROLE_ID, PREMIUM_PLUS_ROLE_ID, LIFETIME_ROLE_ID, LIFETIME_PLUS_ROLE_ID)
// are imported from '../../shared/premium'
...
function isPremium(guild: Guild | null): boolean {
  ...
  if (guild.id === SUPPORT_SERVER_ID) {
    // Support server is ALWAYS premium by default
    return true;
    ...
    return botMember.roles.cache.has(PREMIUM_ROLE_ID) || 
           ...
           botMember.roles.cache.has(LIFETIME_ROLE_ID) ||
           botMember.roles.cache.has(LIFETIME_PLUS_ROLE_ID);
    */
  }
...
function isPremiumPlus(guild: Guild | null): boolean {
  ...
  // Check if this is the support server (where roles are applied)
  if (guild.id === SUPPORT_SERVER_ID) {
    // Support server is ALWAYS premium plus by default
    return true;
