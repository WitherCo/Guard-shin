...
def create_proper_setup_function(class_name):
    """Create a proper setup function for Discord.py 2.0+"""
    return """
...
def setup(bot):
    ...
    bot._BotBase__cogs[{0}.__name__] = {0}(bot)
""".format(class_name)
...
def fix_file(filename):
    ...
    try:
        ...
        # Remove all existing setup functions
        # Look for both regular and async versions
        content = re.sub(r'\s+# Proper setup function for Discord\.py extension loading\s+def setup\(bot\):.*?bot\._BotBase__cogs\[[^\]]+\] = [^(]+\(bot\)', 
                         '', content, flags=re.DOTALL)
        content = re.sub(r'\s+# Proper setup function for Discord\.py extension loading\s+def setup\(bot\):.*?bot\.add_cog\([^)]+\)', 
                         '', content, flags=re.DOTALL)
        content = re.sub(r'\s+# Function to set up the cog\s+async def setup\(bot\):.*?await bot\.add_cog\([^)]+\)', 
                         '', content, flags=re.DOTALL)
