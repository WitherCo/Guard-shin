...
export const PAYMENT_INFO = {
  ...
  LIFETIME_PRICE: 149.99,      // Lifetime Premium one-time payment
  LIFETIME_PLUS_PRICE: 249.99, // Lifetime Premium Plus one-time payment
  PAYMENT_INSTRUCTIONS: `
    ...
    4. Specify which tier you're purchasing (Premium, Premium+, Lifetime, or Lifetime+)
...
export const TIER_FEATURES: Record<SubscriptionTier, PremiumFeature[]> = {
  ...
  [SubscriptionTier.LIFETIME_PLUS]: [
    PREMIUM_FEATURES.ANTI_ALT,
    PREMIUM_FEATURES.CUSTOM_COMMANDS,
    PREMIUM_FEATURES.ADVANCED_AUTOMOD,
    PREMIUM_FEATURES.RAID_PROTECTION_PLUS,
...
export function getTierFromRole(roles: string[]): SubscriptionTier {
  ...
  if (roles.includes(LIFETIME_PLUS_ROLE_ID)) {
    return SubscriptionTier.LIFETIME_PLUS;
  }
...
export function getPaymentUrl(tier: SubscriptionTier, serverId: string, isYearly: boolean = false): string {
  ...
  if (tier === SubscriptionTier.LIFETIME) {
    price = PAYMENT_INFO.LIFETIME_PRICE;
  } else if (tier === SubscriptionTier.LIFETIME_PLUS) {
    price = PAYMENT_INFO.LIFETIME_PLUS_PRICE;
  }
